...
const ImpTotModule = {
          ...
          <div class="table-container mb-none" style="max-height: 260px; overflow-y: auto;">
            <table class="data-table" id="custLinesTable" style="font-size: 11.5px;">
              ...
              <tbody id="custLinesBody">
                ${costLines.map((l, idx) => `
                  <tr data-line-id="${l.id}" data-formula="${l.formulaType}" data-rate-field="${l.rateField}" data-multiplier="${l.multiplier || 1}">
        ...
        <div class="modal-header flex justify-between items-center" style="border-bottom: 1px solid var(--border-subtle); padding: 14px 24px; background: var(--bg-secondary);">
          <div>
            ...
            <div class="text-secondary" style="font-size: 12px; margin-top: 3px;" id="impModalSubtitle">
              Template-Driven Scale Engine &bull; Select or deselect line items &bull; Dynamic Benchmark Unit Rates
            </div>
          </div>
          ...
          <form id="impEventForm">
            ...
            <div class="card p-md mb-md" style="background: var(--bg-secondary); border: 1px solid var(--border-default); box-shadow: var(--shadow-sm);">
              <div class="flex items-center justify-between mb-sm" style="border-bottom: 1px solid var(--border-subtle); padding-bottom: 6px;">
                <div class="font-bold" style="font-size: 11.5px; text-transform: uppercase; letter-spacing: 0.05em; color: var(--accent-primary);">
                  1. 5-Dimensional Budget Tagging & Activity Selection
                </div>
            ...
            <div class="card p-md mb-md" style="background: var(--bg-secondary); border: 1px solid var(--border-default); box-shadow: var(--shadow-sm);">
              <div class="flex justify-between items-center mb-sm" style="border-bottom: 1px solid var(--border-subtle); padding-bottom: 6px;">
                ...
                <div id="impRateNotice" class="badge badge-cyan" style="font-size: 11px; padding: 4px 10px;">
                  📍 Loaded Benchmark Rates: <strong>${rates.stateName || defaultLocation}</strong>
                </div>
              </div>
              ...
              <div class="flex justify-between items-center mb-sm" style="border-bottom: 1px solid var(--border-subtle); padding-bottom: 6px; flex-wrap: wrap; gap: 8px;">
                <div>
                  ...
                  <div class="text-tertiary" style="font-size: 11px;">
                    Select or deselect line items to include/exclude from the calculation. (All selected by default ✅)
                  </div>
                </div>
                <div class="flex items-center gap-xs">
                ...
                <table class="data-table" id="modalCostItemsTable" style="font-size: 12px; width: 100%;">
                  <thead>
                    <tr>
                      ...
                      <th style="min-width: 230px;">Cost Line Item Description</th>
                      <th style="min-width: 140px;">GL Account Code</th>
                      <th style="min-width: 220px;">Calculation Basis / Unit Formula</th>
                      <th class="num" style="min-width: 90px;">Unit Rate (₹)</th>
                      <th class="num font-bold" style="min-width: 130px;">Allocated Amount (${entity.currency})</th>
  ...
  async onLocationChanged(newLoc) {
    ...
    if (notice) {
      notice.innerHTML = `Loaded Benchmark Rates: <strong>${rates.stateName || newLoc}</strong>`;
    }
    this.recalculateModalCost();
  },
  ...
  async recalculateModalCost(compIdOverride = null, ratesOverride = null, customLinesOverride = null, existingSavedLines = null) {
    ...
    const activityName = actSelect ? actSelect.value : '10.1-Bundled ToT- Master trainers';

    // Fetch active Admin Template for this activity
    const template = await db.getImpActivityTemplate(activityName);
    const customFields = await db.getAllImpCustomRateFields();
    ...
    if (existingSavedLines && existingSavedLines.length > 0 && Object.keys(lineSelectionMap).length === 0) {
      existingSavedLines.forEach(sl => {
        if (sl.id) lineSelectionMap[sl.id] = (sl.isSelected !== false);
      });
    }
    ...
    templateLines.forEach(item => {
      ...
      switch (item.formulaType) {
        ...
        case 'events_rate_dual':
          calculatedAmount = events * multiplier * (unitRate + secUnitRate);
          basisText = `${events} Event${events === 1 ? '' : 's'} × (Banner ₹${Utils.formatNumber(unitRate)} + Backdrop ₹${Utils.formatNumber(secUnitRate)})`;
          break;

        case 'events_days_hall_catering':
          calculatedAmount = (events * days * unitRate) + (events * days * participants * secUnitRate);
          basisText = `${events} Event${events === 1 ? '' : 's'} × ${days} Days × (Hall ₹${Utils.formatNumber(unitRate)} + ${participants} Trainees Catering ₹${Utils.formatNumber(secUnitRate)})`;
          break;
    ...
    calculatedLines.forEach(l => {
      ...
      html += `
        <tr class="auto-cost-row ${isMuted ? 'text-muted' : ''}" data-line-id="${l.id}" data-line-desc="${l.description}" data-gl-code="${l.ledgerCode}" data-parent-acc="${l.parentAccount}" data-basis="${l.basis}" data-rate="${l.unitRate}" data-amount="${l.amount}" data-formula="${l.formulaType}" style="${isMuted ? 'opacity: 0.45; background: rgba(0,0,0,0.1);' : ''}">
          <td style="text-align: center;">
          ...
          <td style="text-align: left;">
            <code>${l.ledgerCode}</code>
            <span style="font-size: 10px; color: var(--text-secondary);">${l.parentAccount}</span>
          </td>
    ...
    customLines.forEach((cl, idx) => {
      ...
      html += `
        <tr class="custom-row" id="customRow_${idx}" style="background: rgba(99, 102, 241, 0.04); ${!isSelected ? 'opacity: 0.45;' : ''}">
          ...
          <td style="text-align: left;">
            <select class="form-select custom-gl-select" style="font-size: 11px; padding: 4px 6px;">
              <option value="93201" ${cl.glCode === '93201' ? 'selected' : ''}>93201 Other Direct Expenses</option>
              <option value="93701" ${cl.glCode === '93701' ? 'selected' : ''}>93701 Professional Charges</option>
              <option value="93401" ${cl.glCode === '93401' ? 'selected' : ''}>93401 Office Expenses</option>
              <option value="93105" ${cl.glCode === '93105' ? 'selected' : ''}>93105 Travel & Lodging</option>
              <option value="93204" ${cl.glCode === '93204' ? 'selected' : ''}>93204 Supplies & Printing</option>
            </select>
          </td>
          <td style="text-align: left;">
  ...
  addCustomCostLineInModal() {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align: left;">
        <select class="form-select custom-gl-select" style="font-size: 11px; padding: 4px 6px;">
          <option value="93201">93201 Other Direct Expenses</option>
          <option value="93701" selected>93701 Professional Charges</option>
          <option value="93401">93401 Office Expenses</option>
          <option value="93105">93105 Travel & Lodging</option>
          <option value="93204">93204 Supplies & Printing</option>
        </select>
      </td>
      <td style="text-align: left;">
  ...
  async saveEventFromModal(compKey, editEventId = null) {
    try {
      ...
      // Collect auto lines with isSelected checkbox status
      document.querySelectorAll('#modalCostItemsBody tr.auto-cost-row').forEach(r => {
        ...
        costLines.push({
          ...
          ledgerCode: r.dataset.glCode,
          parentAccount: r.dataset.parentAcc,
      ...
      document.querySelectorAll('#modalCostItemsBody tr.custom-row').forEach(r => {
        ...
        const customItem = {
          ...
          description: desc,
          ledgerCode: glCode,
          parentAccount: glCode.startsWith('931') ? 'Travel & Lodging Expenses' : (glCode.startsWith('937') ? 'Professional & Consultancy Charges' : (glCode.startsWith('934') ? 'Office Expenses' : 'Supplies & Printing Costs')),
          basis: basis,
          unitRate: 0,
          amount: isSelected ? amt : 0,
          originalCalculatedAmount: amt
